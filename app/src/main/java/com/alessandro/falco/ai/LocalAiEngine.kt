...
data class AiSuggestion(val genre: String?, val subgenre: String?, val energy: Int, val rating: Int?, val confidence: Int, val neighbors: Int)

class LocalAiEngine(context: Context) {
    private val prefs = context.getSharedPreferences("falco_local_ai", Context.MODE_PRIVATE)
    private data class Sample(val features: FloatArray, val genre: String, val subgenre: String, val energy: Int, val rating: Int)
    ...
    fun suggest(track: TrackEntity, waveform: List<Float>): AiSuggestion {
        ...
        if (nearest.size < 3) return AiSuggestion(null, null, energyFromAudio, null, (25 + nearest.size * 10).coerceAtMost(50), nearest.size)
        ...
        return AiSuggestion(genre, subgenre, energy, rating, (45 + agreement * 45).toInt(), nearest.size)
    }

    fun learn(track: TrackEntity, waveform: List<Float>, genre: String, subgenre: String, energy: Int, rating: Int) {
        if (waveform.isEmpty() || genre.isBlank() || rating == 0) return
        val all = load().takeLast(999).toMutableList().apply { add(Sample(features(track, waveform), genre, subgenre, energy.coerceIn(1, 5), rating.coerceIn(1, 5))) }
        val json = JSONArray(); all.forEach { s -> json.put(JSONObject().put("f", JSONArray(s.features.toList())).put("g", s.genre).put("s", s.subgenre).put("e", s.energy).put("r", s.rating)) }
    ...
    private fun load(): List<Sample> = runCatching { val arr = JSONArray(prefs.getString("samples", "[]")); List(arr.length()) { i -> val o = arr.getJSONObject(i); val f = o.getJSONArray("f"); Sample(FloatArray(f.length()) { f.getDouble(it).toFloat() }, o.getString("g"), o.optString("s", o.getString("g")), o.getInt("e"), o.getInt("r")) } }.getOrDefault(emptyList())
}
