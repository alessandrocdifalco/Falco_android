...
interface TrackDao {
    ...
    @Query("SELECT * FROM tracks WHERE (maestCache = '' AND aiAnalysisError = '') OR aiSourceModifiedAt != modifiedAt ORDER BY CASE WHEN workStatus = 'DA_VALUTARE' THEN 0 ELSE 1 END, modifiedAt DESC LIMIT :limit")
    suspend fun pendingAnalysis(limit: Int): List<TrackEntity>
    @Query("SELECT COUNT(*) FROM tracks WHERE (maestCache = '' AND aiAnalysisError = '') OR aiSourceModifiedAt != modifiedAt") suspend fun pendingAnalysisCount(): Int
